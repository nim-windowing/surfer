import sets
import bitops
import pkg/surfer/backend/wayland/bindings/vulkan
...
proc findQueueFamilies(
  ...
  for queueFamily in queueFamilies:
    ...
    discard
      vkGetPhysicalDeviceSurfaceSupportKHR(pDevice, index, surface, presentSupport.addr)
...
proc createLogicalDevice(
    ...
    graphicsQueue: var VkQueue,
    presentQueue: var VkQueue,
): VkDevice =
  let
...
proc querySwapChainSupport(
  ...
  if presentModeCount != 0:
    ...
    discard vkGetPhysicalDeviceSurfacePresentModesKHR(
      pDevice, surface, presentModeCount.addr, result.presentModes[0].addr
    )
...
proc createInstance(
    glfwExtensions: cstringArray, glfwExtensionCount: uint32
): VkInstance =
  ...
  discard
    vkEnumerateInstanceExtensionProperties(nil, extensionCount.addr, extensions[0].addr)
...
proc createSwapChain(
    device: VkDevice, physicalDevice: VkPhysicalDevice, surface: VkSurfaceKHR
): SwapChain =
  let
  ...
  var createInfo = VkSwapchainCreateInfoKHR(
    sType: cast[VkStructureType](1000001000),
  ...
  createInfo.oldSwapChain = swapchain.swapchain # VkSwapchainKHR(0)
  if vkCreateSwapChainKHR(device, createInfo.addr, nil, result.swapChain.addr) !=
      VK_SUCCESS:
  ...
  discard vkGetSwapchainImagesKHR(
    device, result.swapChain, imageCount.addr, result.swapChainImages[0].addr
  )
  result.swapChainImageFormat = surfaceFormat.format
  result.swapChainExtent = extent

proc createImageViews(
    device: VkDevice, swapChainImages: seq[VkImage], swapChainImageFormat: VkFormat
): seq[VkImageView] =
...
proc createGraphicsPipeline(
    device: VkDevice, swapChainExtent: VkExtent2D, renderPass: VkRenderPass
): GraphicsPipeline =
  const
  ...
  var
    ...
    scissor = VkRect2D(offset: VkOffset2D(x: 0, y: 0), extent: swapChainExtent)
  ...
  if vkCreatePipelineLayout(
    device, pipelineLayoutInfo.addr, nil, result.pipelineLayout.addr
  ) != VK_SUCCESS:
...
proc createRenderPass(device: VkDevice, swapChainImageFormat: VkFormat): VkRenderPass =
  var
    ...
    dependency = VkSubpassDependency(
      ...
      dstSubpass: 0,
      srcStageMask: VkPipelineStageFlags(0x00000400),
        #VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT
      srcAccessMask: VkAccessFlags(0),
      dstStageMask: VkPipelineStageFlags(0x00000400),
        #VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT
...
proc createFramebuffers(
  ...
  for i in 0 ..< swapChainImageViews.len:
    ...
    if vkCreateFramebuffer(device, framebufferInfo.addr, nil, result[i].addr) !=
        VK_SUCCESS:
...
proc createCommandBuffers(
  ...
  for i in 0 ..< result.len:
    ...
    var
      clearColor = VkClearValue(color: VkClearColorValue(float32: [0f, 0f, 0f, 1f]))
...
proc createSemaphores(device: VkDevice): Semaphores =
  ...
  if vkCreateSemaphore(device, semaphoreInfo.addr, nil, result.imageAvailable.addr) !=
      VK_SUCCESS or
      vkCreateSemaphore(device, semaphoreInfo.addr, nil, result.renderFinished.addr) !=
      VK_SUCCESS:
    quit("failed to create semaphores")

proc init*(surf: VkSurfaceKHR, inst: VkInstance) =
  surface = surf
  instance = inst
  ...
  swapChainImageViews =
    createImageViews(device, swapChain.swapChainImages, swapChain.swapChainImageFormat)
  renderPass = createRenderPass(device, swapChain.swapChainImageFormat)
  graphicsPipeline =
    createGraphicsPipeline(device, swapChain.swapChainExtent, renderPass)
  swapChainFramebuffers = createFramebuffers(
    device, swapChain.swapChainExtent, swapChainImageViews, renderPass
  )
  commandPool = createCommandPool(device, physicalDevice, surface)
  commandBuffers = createCommandBuffers(
    device, swapChain.swapChainExtent, renderPass, graphicsPipeline.pipeline,
    swapChainFramebuffers, commandPool,
  )
...
proc tick*() =
  ...
  if vkQueueSubmit(graphicsQueue, 1, submitInfo.addr, VkFence(VK_NULL_HANDLE)) !=
      VK_SUCCESS:
